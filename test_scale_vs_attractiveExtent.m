...
scales = 0.7:0.3:4.9;
ra = ra .* scales;
ra(:,1:2:end) = ra(:,1:2:end) + 2.*scales(1:2:end);
scales = scales .* ones(size(ra));

ra = ra(:);
scales = scales(:);

r0 = round(ra*0.12);
rd = -1 + max(0,log10(ra/100));
...
save([pth 'resultsInfo_scale_ra_scan_' datestr(now,'yyyymmddTHHMMSS') '.mat'], 'Info','options')
...
dims.dN = -3:3;
dims.params = params;
dims.paramsDisp = {'ra','scale','r0','rd'};
...
save([pth 'results_scale_ra_scan_' datestr(now,'yyyymmddTHHMMSS') '.mat'], 'results')