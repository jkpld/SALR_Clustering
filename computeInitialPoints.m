...
% r0set : a set of possible initial points. This parameter is required if
...
%
% Output parameters:
% r0 : The initial particle locations. If options.Iterations > 1, then r0
%   is a cell array with length options.Iterations where each element is an
%   NxD array of initial positions. If options.Iterations = 1, then r0
%   is an NxD array.
...
function [sz, lattice_constant, N, r0set, BWpts, offset, use_cellarray] = parse_inputs(BW, options, varargin)
    ...
    lattice_constant = rs * (particle_size/cell_size)^(1/D);
...
function [r0_set, varargout] = uniform(BW, lattice_constant, repetitions)
    ...
    if D == 2
        ...
        a = 2*lattice_constant(1); % Use the first value for the size of the hexagons
        x = a/2 : a : sz(2)+a;
