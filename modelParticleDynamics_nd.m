function [r, varargout] = modelParticleDynamics_nd(V,r0,options)
...
odeFun = @(t,y) interactingParticleSystem_nd(t,y,SystemInputs);

% Initialize ode history
hstry = ode_history(SOLVER_TIME_RANGE(1), y0, HISTORY_SIZE);

% Set up the solver options.
ode_options = odeset('Events',@(t,y) interactingParticleSystem_convergeEvent_nd(t,y,m,hstry,D),...
    'Vectorized','on',...
    'RelTol',1e-4,...
    'AbsTol',1e-6,...
    'NormControl','on');%,...
