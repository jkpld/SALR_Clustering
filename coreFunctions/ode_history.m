classdef ode_history < handle
    % ODE_HISTORY Handle based class for storing an ode solution at
    % previous time steps while solving. Use the history in an events
    % function to catch more complex behavior than can be determined with
    ...
    %
    % hardreset(obj, t, sol) reset the history by setting iteration = 1 and
    % solHistory(:,end) = sol, time(end) = t;
    %
    % rewrite(obj, ts, sols, offset) - completely rewrite the history using
    % time = ts, solHistory = sols, and iteration = interation - offset;
